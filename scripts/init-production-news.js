...
/**
 * Initialize production news data in Supabase
 * This script ensures the news_articles table exists and has sample data
 */

const { createClient } = require('@supabase/supabase-js');
require('dotenv').config({ path: '../website/.env.production' });
...
if (!supabaseUrl || !supabaseKey) {
  ...
  console.error('Please set VITE_SUPABASE_URL and VITE_SUPABASE_SERVICE_ROLE_KEY in .env.production');
  process.exit(1);
}

const supabase = createClient(supabaseUrl, supabaseKey);

// Sample news data for production
const sampleNewsData = [
  ...
  },
  {
    ...
    content_en: "The Office of the Attorney General has launched a comprehensive digital transformation initiative aimed at modernizing legal service delivery across Kenya. The new digital platforms include online legal consultation services, digital document processing, and AI-powered legal assistance through the innovative SheriaBot system. This technological advancement ensures that all Kenyans, regardless of their location, can access quality legal services and information about their rights and obligations under the law.",
    content_sw: "Ofisi ya Mwanasheria Mkuu imeanzisha mpango mkuu wa mabadiliko ya kidijitali unaolenga kumodernisha utoaji wa huduma za kisheria nchini Kenya. Mifumo mpya ya kidijitali inajumuisha huduma za ushauri wa kisheria mtandaoni, uchakataji wa hati za kidijitali, na msaada wa kisheria unaotumia akili bandia kupitia mfumo wa SheriaBot. Maendeleo haya ya kiteknolojia yanahakikisha kwamba Wakenya wote, bila kujali mahali walipo, wanaweza kupata huduma bora za kisheria na habari kuhusu haki na majukumu yao chini ya sheria.",
    status: 'published',
    is_featured: false,
    published_at: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000).toISOString(),
    tags: ['digital-transformation', 'technology', 'legal-services', 'innovation']
  },
  {
    ...
    content_en: "The Office of the Attorney General continues to strengthen Kenya's position in international legal affairs through active participation in global legal forums and bilateral cooperation agreements. Recent engagements include collaboration with international courts, participation in treaty negotiations, and sharing of best practices in constitutional law and human rights protection. These efforts position Kenya as a leader in African legal development and contribute to the advancement of international law principles.",
    content_sw: "Ofisi ya Mwanasheria Mkuu inaendelea kuimarisha nafasi ya Kenya katika mambo ya kisheria ya kimataifa kupitia ushiriki mkamilifu katika mikutano ya kisheria ya kimataifa na makubaliano ya ushirikiano wa kipande mbili. Mahusiano ya hivi karibuni yanajumuisha ushirikiano na mahakama za kimataifa, ushiriki katika mazungumzo ya mikataba, na kushiriki mbinu bora za sheria za kikatiba na ulinzi wa haki za binadamu. Juhudi hizi zinaiweka Kenya kama kiongozi katika maendeleo ya kisheria ya Afrika na kuchangia maendeleo ya kanuni za sheria za kimataifa.",
    status: 'published',
    is_featured: false,
    published_at: new Date(Date.now() - 4 * 24 * 60 * 60 * 1000).toISOString(),
    tags: ['international-law', 'diplomacy', 'cooperation', 'treaties']
  }
];

async function initializeNewsData() {
  console.log('🚀 Initializing production news data...');
  ...
  try {
    ...
    if (countError) {
      console.error('❌ Error checking news_articles table:', countError.message);
      return;
    }

    console.log(`📊 Found ${count || 0} existing articles`);
    ...
    for (const article of sampleNewsData) {
      ...
      if (error) {
        ...
        console.log(`✅ Inserted: ${article.title_en}`);
      }
    }
    ...
    console.error('❌ Failed to initialize news data:', error.message);
    process.exit(1);
  }
}
